...
func (optr *Operator) syncMachineConfiguration(_ *renderConfig, _ *configv1.ClusterOperator) error {
	...
	supportsBootImageUpdates := ctrlcommon.CheckBootImagePlatform(infra, optr.fgHandler)
...
func (optr *Operator) syncBootImageSkewEnforcementStatus(mcop *opv1.MachineConfiguration, newMachineConfigurationStatus *opv1.MachineConfigurationStatus, supportsBootImageUpdates bool) {
	...
	if mcop.Spec.BootImageSkewEnforcement != (opv1.BootImageSkewEnforcementConfig{}) {
		...
		if reflect.DeepEqual(newMachineConfigurationStatus.ManagedBootImagesStatus, apihelpers.GetManagedBootImagesWithUpdateEnabled()) {
