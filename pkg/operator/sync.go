...
// that is generated by the current version of the MCO and that the master pool atleast has 1
// upgraded node(or has completed an upgrade). This "stamp" is used by the machine set controller
// as a safety before it updates boot images.

func (optr *Operator) stampBootImagesConfigMap() error {
	// Grab master pool
	pool, err := optr.mcpLister.Get(ctrlcommon.MachineConfigPoolMaster)
	if err != nil {
		return fmt.Errorf("failed to grab master pool, error: %w", err)
	}
...
func (optr *Operator) syncManagedBootImagesStatus(mcop *opv1.MachineConfiguration, status *opv1.MachineConfigurationStatus, isDefaultOnPlatform, supportCPMSBootImageUpdates bool) bool {
	...
	if mcop.Spec.ManagedBootImages.MachineManagers != nil && apihelpers.HasMAPIMachineSetManager(mcop.Spec.ManagedBootImages.MachineManagers, opv1.MachineSets) {
		...
		mapiMode = apihelpers.GetMAPIMachineSetManagerMode(mcop.Spec.ManagedBootImages.MachineManagers, opv1.MachineSets)
	...
	apihelpers.MergeMAPIManagerMode(status, opv1.MachineSets, mapiMode)
	...
	if optr.fgHandler.Enabled(features.FeatureGateManagedBootImagesCPMS) && supportCPMSBootImageUpdates {
		...
		if mcop.Spec.ManagedBootImages.MachineManagers != nil && apihelpers.HasMAPIMachineSetManager(mcop.Spec.ManagedBootImages.MachineManagers, opv1.ControlPlaneMachineSets) {
			...
			cpmsMode = apihelpers.GetMAPIMachineSetManagerMode(mcop.Spec.ManagedBootImages.MachineManagers, opv1.ControlPlaneMachineSets)
		}
		// Merge the determined ControlPlaneMachineSet Manager mode to status
		apihelpers.MergeMAPIManagerMode(status, opv1.ControlPlaneMachineSets, cpmsMode)
	}
...
func (optr *Operator) syncPreBuiltImageMachineConfigs() error {
	...
	for _, mosc := range moscs {
		...
		// Check if this MOSC was seeded with a pre-built image
		...
		// Strategy: Preserve existing MC, otherwise use annotation to create it
		// This ensures component MCs are never deleted or updated once created
		if existingMC, exists := existingPreBuiltMCs[poolName]; exists {
			// Component MC exists - preserve it using its current osImageURL (frozen forever)
			...
			// Component MC doesn't exist yet, but annotation is present - create the MC
		...
		} else if hasSeededCondition {
			// ERROR: MC missing for a pool that was seeded with pre-built image
			// The MC should exist since seeding completed - it may have been manually deleted
			return fmt.Errorf("MachineOSConfig %q for pool %q was seeded with a pre-built image but the component MachineConfig is missing - this is required for install-time OCL", mosc.Name, poolName)
		}
		// else: No MC, no annotation, not seeded - this pool doesn't use pre-built images, skip
